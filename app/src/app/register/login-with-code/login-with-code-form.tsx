...
export function LoginWithCodeForm({
  ...
  const color = branding?.color || '#0047AB'
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    // Set site session cookie
    const sessionValue = slug || 'main'
    document.cookie = `site_session=${sessionValue};path=/;max-age=${60 * 60 * 24 * 365};samesite=lax`

    // If we have a slug + code, add this user to program_members
    if (slug && code) {
      const res = await fetch('/api/auth/join-with-invite', {
        method: 'POST',
      ...
      if (!res.ok) {
        setError(data.error || 'Could not join program. Please try again.')
        setLoading(false)
        return
      }
  ...
  const handleGoogleSignIn = async () => {
    ...
    // existing=1 tells auth/callback to add them to program_members directly, skip profile-setup
    const callbackUrl = slug && code
      ? `${getAppUrl()}/auth/callback?slug=${encodeURIComponent(slug)}&invite_code=${encodeURIComponent(code)}&existing=1`
    ...
    >
      ...
      <div className="relative z-10 w-full max-w-md">
        ...
        <div className="bg-white/90 backdrop-blur-sm rounded-2xl shadow-xl p-7 border border-white">
          ...
          <p className="text-sm text-gray-500 mb-5">Sign in to join the program.</p>
        ...
        <p className="mt-5 text-center text-sm text-gray-500">
          ...
          <Link
            href={slug && code ? `/register?slug=${encodeURIComponent(slug)}&code=${encodeURIComponent(code)}` : '/register'}
