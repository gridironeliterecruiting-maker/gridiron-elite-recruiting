...
export async function GET() {
  ...
  try {
    ...
    // Use Zoho's Threads API — returns pre-grouped conversations, no manual threading needed
    const res = await zohoFetch(
      `${ZOHO_API_BASE}/accounts/${accountKey}/threads?folderId=${inboxFolderId}&limit=50&sortby=date&sortorder=false`,
      {}
    )

    if (!res.ok) {
      const err = await res.text()
      console.error('[email/inbox] Zoho threads error:', res.status, err)
      return NextResponse.json({ threads: [], unreadCount: 0 })
    }
    ...
    const threads = rawThreads.map((t: any) => {
      ...
      // The "other person" is whoever is NOT the athlete
